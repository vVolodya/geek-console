...
const searchBooks = async (e) => {
  ...
  if (!books.length) {
    ...
    books.forEach((book) => {
      booksHtml += `
      <div class="flex flex-col justify-center items-center w-full rounded-lg border shadow-md bg-[#232323] border-[#2e2e2e] p-4">
        <a href=${book.url} class="text-center" target="_blank" rel="noopener noreferrer">
          <img class="w-40 h-40 max-w-full align-middle rounded-full" src=${book.photo ? book.photo : '../images/noCover.gif'} alt=${book.title} />
        </a>
        <div class="p-5 flex flex-col justify-center items-center">
          ...
          <div class="flex justify-between gap-4">
            <a href=${book.url} class="py-2 px-3 text-sm font-bold text-center text-gray-900 rounded-lg focus:outline-none bg-[#F7BE38] hover:bg-[#F7BE38]/80 focus:ring-yellow-800" target="_blank" rel="noopener noreferrer">
              Read more
            </a>
            <button data-id=${book.id} type="button" class="font-bold border focus:ring-4 focus:outline-none rounded-lg text-sm px-3 py-2 text-center border-yellow-300 text-yellow-300 hover:bg-yellow-300/20 focus:ring-yellow-900">Remove</button>
          </div>
        </div>
      </div>
      `;
